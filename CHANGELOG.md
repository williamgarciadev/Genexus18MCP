...
- **`genexus_introspect` — reconnaissance before the magnifying glass.** Call it first
  ...
  report".
- **`genexus://kb/skills/clean-architecture` — the team coding standard, served by the MCP
  itself.** A normative document (in Spanish): SOLID translated honestly to non-OOP GeneXus
  (what applies, what does not — Liskov is declared out, with the closest real substitutes),
  ACID/LUW rules with verified GeneXus behaviours (Rollback does not apply in Transactions;
  Business Components do not commit on their own; there is no SNAPSHOT isolation value),
  size limits per object role, the canonical `parm(in:&Sdt, out:&Resultado, out:&Mensajes)`
  signature, single-commit ownership for batch flows, a code-smell catalog aligned
  one-to-one with the linter rules, and a blocking Definition of Done. The document is
  embedded into the Gateway assembly at build time, so it reaches every machine the
  binaries reach — human developers and AI agents alike. Master copy:
  `docs/clean-architecture-genexus.md`.
- **`GX014` — Long Procedure Source (Warning).** Fires when a Procedure exceeds 80 useful
  code lines (non-blank after comment stripping), the team limit for a main procedure.
  The message points at the extraction targets (validation ≤60, save ≤50, interop ≤30).
- **`GX015` — Commented-Out Code Block (Warning).** Flags blocks of more than 10
  consecutive commented-out CODE lines — prose and change-history comments do not fire
  (a code-likeness ratio discriminates). Regions generated by WorkWithPlus
  (`/* Generated by DVelop ... [Start] */` … `[End]`, marker format verified against a
  real KB) are excluded: that code is regenerated on the next F9 and is not the
  developer's to delete.
...
- **`GX012` (Direct Table Access in UI) raised from Info to Warning.** At Info it was
  ignored, and each new screen copied the `For Each` from the previous one. As a Warning
  it now requires a written justification (`// GX012-justified: <reason>`) under the
  standard's severity policy. The WorkWithPlus suppression is unchanged — objects with a
  pattern instance are still exempt, since the pattern itself prescribes direct access in
  its generated events.

### Internal

- The severity policy is now normative and documented in `docs/linter_rules.md`:
  Critical/Error blocks, Warning requires a written `// GXnnn-justified:` note, Info logs.
  GX015's detection lives in the pure helper `Helpers/CommentedCodeDetector.cs`
  (thresholds pinned by `CommentedCodeDetectorTests`); the 80-line count lives in
  `CodeMetricsExtractor.CountCodeLines` so the definition of "useful line" has one home.
  GX007 was not recycled for the new rules — it existed historically and would poison
  old reports; GX016+ stay reserved for the remaining size limits once object-role
  inference exists.
